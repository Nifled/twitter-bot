...
class MarkovBot():
	...
	def generate_text(self):
		# arguments  maxlength, seedword=None, database=u'default', verbose=False, maxtries=100
		listy = ['khe', 'berga', 'oiekhe' ,'ke', 'pedo', 'when', 'tu', 'mama' ,'kp2', \
		'jachiros', 'machin lernin', 'meme' , 'spooky', 'qlo', 'pos oie', 'pos', 'tons', \
		'prro', 'jalowin', 'saca', 'asopotamadre', 'saka saka', 'quica', 'anuma', 'numaaaaa', \
		'dorislokos', 'ke dijistes del amerika we']
		result = ''
		for x in range(10):
			word = random.randrange(0,len(listy))
			if listy[word] in result:
				continue
			result += listy[word] + ' '
		return result
